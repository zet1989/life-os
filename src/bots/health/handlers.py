...
async def _watch_context(user_id: int) -> str:
    ...
    return await watch_context(user_id)
...
PROFILE_MERGE_SYSTEM = (
    ...
    "4. СОХРАНИ ВСЮ информацию из старого профиля ПОЛНОСТЬЮ и ДОСЛОВНО. "
    "НЕ сокращай, НЕ обрезай даты, НЕ удаляй строки, НЕ пересказывай своими словами. "
    "Каждое предложение из старого профиля ДОЛЖНО быть в результате.\n"
    "5. Верни ТОЛЬКО итоговый текст профиля. ЗАПРЕЩЕНО добавлять свои мысли, "
    "комментарии, пояснения, фразы типа «Буду учитывать...» или «Принято к сведению».\n"
    "6. Пиши на русском. Формат — краткий структурированный текст от третьего лица.\n"
    "7. Если НОВЫЕ СВЕДЕНИЯ — это вопрос, просьба, команда или НЕ содержат "
    "никаких новых фактов о пользователе — ответь РОВНО ОДНИМ словом: [NO_CHANGES]\n"
    "8. НЕ ВЫДУМЫВАЙ информацию. Если чего-то нет в исходных данных — не добавляй."
)
...
async def _process_profile(message: Message, user_id: int, text: str) -> None:
    ...
    # Если LLM определил, что сообщение — вопрос/просьба, а не данные профиля
    if "[NO_CHANGES]" in merged:
        set_user_mode(user_id, Mode.FOOD)
        await _process_food_text(message, user_id, text)
        return

    # Очистка от AI-отсебятины (фразы, которые LLM добавляет от себя)
    _ai_junk = [
        "Буду учитывать", "Принято к сведению", "Информация сохранена",
        "Профиль обновлён", "Данные добавлены", "Учту в работе",
        "Буду использовать", "Принял к сведению",
    ]
    for junk in _ai_junk:
        for line in merged.split("\n"):
            if junk.lower() in line.lower() and len(line.strip()) < 80:
                merged = merged.replace(line, "")
    merged = re.sub(r"\n{3,}", "\n\n", merged).strip()

    # Защита от потери данных: если merged короче existing — LLM обрезал
    if existing and len(merged) < len(existing) * 0.95:
        logger.warning(

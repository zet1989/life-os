...
### 20 апреля 2026 — Фикс привычек: процент в обзоре + серия по дням

- **Проблема 1:** еженедельный обзор показывал 0% для всех привычек — `progress_pct` статическое поле в БД (DEFAULT 0), не вычислялось из чекинов.
- **Решение 1:** новый SQL-запрос `get_weekly_habit_progress(user_id, goal_id)` — считает уникальные дни с success за текущую неделю, вычисляет %. В `_send_weekly_to_user` для `habit_target` целей используется авторасчёт вместо статического поля.
- **Проблема 2:** серия (streak) привычки показывала 4 дня подряд несмотря на 2-дневный перерыв — считались подряд идущие записи events, без привязки к календарным дням.
- **Решение 2:** streak пересчитан по календарным дням (MSK) — собираются уникальные даты success, затем считаются подряд идущие дни назад от сегодня. Пропуск дня обрывает серию.
- **Бонус-фикс:** `get_monthly_habit_stats` искал `event_type='habit_check'` и `status='done'`, а реально сохраняется `'habit'` и `'success'`.
- **Файлы:** `src/db/queries.py`, `src/bots/master/scheduler.py`, `src/bots/psychology/handlers.py`

...
| # | Бот | Токен env | Назначение |
|---|-----|-----------|------------|
| 1 | **Health** (Нутрициолог + Тренер + Доктор) | `BOT_TOKEN_HEALTH` | Фото еды → КБЖУ + health score + советы; лог тренировок; Доктор (gpt-4o, медконсультации, анализы); профиль пользователя |
| 2 | **Assets** (Дом + Авто) | `BOT_TOKEN_ASSETS` | Бортжурнал авто, замеры дома, чеки → финансы, напоминания по пробегу |
| 3 | **Business** (Мульти-проект) | `BOT_TOKEN_BUSINESS` | Проектный бизнес с per-project промптами, фин. отчёты (SQL only) |
| 4 | **Partner** (Опердир) | `BOT_TOKEN_PARTNER` | Мульти-проект для партнёра Александра, privacy mode |
| 5 | **Mentor** (Стратег) | `BOT_TOKEN_MENTOR` | Идеи → RAG, анализ обсуждений (Whisper), фин. стратегия |
| 6 | **Family** (Семейный бюджет) | `BOT_TOKEN_FAMILY` | Расходы/доходы, OCR чеков, отчёты, лимиты категорий |
| 7 | **Psychology** (Психолог) | `BOT_TOKEN_PSYCHOLOGY` | Дневник → RAG, трекинг привычек, ретроспектива, кросс-бот контекст, профиль |
| 8 | **Master** (Главный) | `BOT_TOKEN_MASTER` | Хранитель видения, фин. панорама, аудит, промпт-менеджмент |

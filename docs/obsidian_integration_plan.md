...
> **Дата:** 07.04.2026  
> **Статус:** Фаза 10 — завершена ✅ | Фаза 9 — завершена ✅  
...
- [x] **9.0.1** Создать Obsidian Vault локально (папка `~/ObsidianVault/LifeOS/`)
- [x] **9.0.2** Определить структуру папок Zettelkasten в Vault:
  ```
  LifeOS/
  ...
  └── 08-Archive/            # Архив
  ```
- [x] **9.0.3** Установить плагины Obsidian: **Tasks**, **Dataview**, **Templater**, **Calendar**, **Periodic Notes**
- [x] **9.0.4** Создать шаблон Daily Note (`07-Templates/Daily.md`):
  ```markdown
  ...
  ```
- [x] **9.0.5** Создать шаблон Meeting Note (`07-Templates/Meeting.md`)
- [x] **9.0.6** Создать MOC-дашборды: `[[Dashboard — Бизнес]]`, `[[Dashboard — Здоровье]]`, `[[Dashboard — Цели]]`

### 9.1 Синхронизация: Syncthing (VPS ↔ Laptop)

- [x] **9.1.1** Установить Syncthing на ноутбук (Windows)
- [x] **9.1.2** Установить Syncthing на VPS (Docker-контейнер либо apt)
- [x] **9.1.3** Настроить синхронизацию папки Vault: `~/ObsidianVault/LifeOS/` ↔ `/opt/life-os/obsidian-vault/`
- [x] **9.1.4** Проверить двустороннюю синхронизацию (создать файл локально → появился на VPS, и наоборот)
- [x] **9.1.5** Добавить `/opt/life-os/obsidian-vault/` в `.gitignore` (не коммитить заметки)
- [x] **9.1.6** Добавить папку obsidian-vault как volume в `docker-compose.yml`:
  ```yaml
  bot:
    volumes:
...
- [x] **9.2.1** Создать модуль `src/integrations/obsidian/__init__.py`
- [x] **9.2.2** Создать `src/integrations/obsidian/config.py` — пути к папкам Vault, настройки
- [x] **9.2.3** Создать `src/integrations/obsidian/writer.py` — базовый класс записи .md файлов:
...
- [x] **9.2.4** Экспорт событий бота → Inbox:
...
- [x] **9.2.8** Интегрировать writer в существующие хэндлеры (вызывать после `create_event`):
...
- [x] **9.3.1** Добавить `watchdog` в `requirements.txt`
- [x] **9.3.2** Создать `src/integrations/obsidian/task_parser.py`:
...
- [x] **9.3.4** Создать SQL-миграцию — новая таблица `obsidian_tasks`:
  ...
  ```
- [x] **9.3.5** Добавить SQL-запросы в `src/db/queries.py`:
...
- [x] **9.3.6** Добавить scheduler-задачу в master-бот (`src/bots/master/scheduler.py`):
...
- [x] **9.3.7** Утренняя сводка задач на день (APScheduler, 08:00 MSK):
...
- [x] **9.4.1** При выполнении задачи в Obsidian (`- [x]`) → watcher обновляет БД (`is_done = TRUE`)
- [x] **9.4.2** Telegram-команда `/done <task_id>` → обновить БД + перезаписать в .md файле `- [ ]` → `- [x] ✅ {date}`
- [x] **9.4.3** Telegram-команда `/task Текст 📅 2026-04-15 ⏰ 10:00` → создать запись в `04-Daily/{today}.md` + БД
- [x] **9.4.4** Reply-кнопка `📋 Задачи` в master-боте → показать задачи на сегодня из `obsidian_tasks`

### 9.5 RAG-интеграция с Obsidian

Obsidian-заметки как дополнительный источник для RAG-поиска.

- [x] **9.5.1** При создании/обновлении заметки в `02-Knowledge/` → генерировать embedding → сохранять в `events` (event_type = `obsidian_note`)
- [x] **9.5.2** Бот Master/Mentor при RAG-поиске учитывает `obsidian_note` события
- [x] **9.5.3** При RAG-ответе, если источник — заметка Obsidian, добавлять ссылку: `📝 Источник: Knowledge/Скрипт продаж B2B.md`
...
- [x] **9.6.1** Добавить в `src/config.py`:
  ...
  ```
- [x] **9.6.2** Добавить переменные в `.env.example`
...
- [x] **10.4.1** Задачи из Telegram → автоматически попадают в `04-Daily/{date}.md`
- [x] **10.4.2** Задачи из Obsidian → автоматически видны в Telegram через `📋 Задачи`
- [x] **10.4.3** Отметка `[x]` в любом месте → синхронизация в обе стороны
